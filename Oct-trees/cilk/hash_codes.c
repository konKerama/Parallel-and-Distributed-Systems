...
void computation(unsigned int *codes, float *X, float *low, float step, int i){
...
void quantize(unsigned int *codes, float *X, float *low, float step, int N){
  ...
  //There are no dependecies so I can make the loop parallel

  cilk_for(int i=0; i<N; i++){

    computation(codes, X, low, step, i);
  }
